...
/**
 * [RemoteMovieReviewSourceImpl] unit test class.
 */
class RemoteMovieReviewSourceImplTest {
    ...
    // Test subject
    private lateinit var source: RemoteMovieReviewSourceImpl

    // Collaborators
    private val api: TheMovieDbApi = mockk()
    private val mapper: Mapper2<MovieDetailResponse, TrailersResponse, Result<MovieReviewEntity>> = mockk()
    private val networkErrorHandler: NetworkErrorHandler = mockk()

    @Before
    fun setUp() {
        // Init test subject
        source = RemoteMovieReviewSourceImpl(api, mapper, networkErrorHandler)
    }
    ...
    fun loadReviewFromRemoteApiWhenQueried() {
        ...
        val mappedResult = mockk<Result<MovieReviewEntity>>()
